...
import logging
logger = logging.getLogger('./web.log').setLevel(logging.WARNING)
print("Inicializando...")
...
try:
    ...
    # MYEDIT available_languages = get_languages()
    ...
    with col3:
        with st.expander("Settings"):
            ...
            #MY EDIT st.selectbox("Language", [None] + list(available_languages.keys()), key='translation_language')
    ...
    if question != '':
        ...
        st.markdown("Answer:" + "Hello, this is not ChatGPT. This is just a placeholder answer. Thank you for you patient while we develop")
        st.markdown('\n\nSources: List of sources')
        '''with st.expander("Question and Answer Context"):
            st.markdown(st.session_state['context'].replace('$', '\$'))
            st.markdown(f"SOURCES: {sources}")'''

    '''MYEDIT if st.session_state['translation_language'] and st.session_state['translation_language'] != '':
